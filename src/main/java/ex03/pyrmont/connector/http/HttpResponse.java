...
//  你传递给PrintWriter的print或println方法的任何东西都是通过底下的OutputStream进行发送的。

//  java.io.OutputStreamWriter对象充当ResponseWriter对象和ResponseStream对象之间的桥梁。
//  通过OutputStreamWriter，写进去的字符通过一种特定的字符集被编码成字节。
//  这种字符集可以使用名字来设定，或者明确给出，或者使用平台可接受的默认字符集。
//  write方法的每次调用都会导致在给定的字符上编码转换器的调用。
//  在写入底层的输出流之前，生成的字节都会累积到一个缓冲区中。
//  缓冲区的大小可以自己设定，但是对大多数场景来说，默认的就足够大了。
//  注意的是，传递给write方法的字符是没有被缓冲的。
