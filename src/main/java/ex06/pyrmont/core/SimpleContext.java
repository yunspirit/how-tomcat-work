...
//SimpleContext类使用如下变量来引用一个LifecycleSupport实例。
...
//  它还使用一个名为started的变量来标记该SimpleContext实例是否已经启动
...
//  addLifecycleListener方法添加一个生命周期事件：
...
//  要删除一个生命周期监听事件，SimpleContext调用LifecycleSupport类的removeLifecycleListener方法。
...
//  SimpleContext实例中注册的监听器都会被唤醒，
//  一个SimpleContextLifecycleListener类型的监听器会注册它“感兴趣”的事件
    lifecycle.fireLifecycleEvent(BEFORE_START_EVENT, null);

//    started布尔变量为真来标记该组件已经启动
    started = true;
//    start方法接下来会启动所有组件以及其子容器
//    现在有两个组件实现了Lifecycle接口：SimpleLoader 和 SimplePipeline。
//    SimpleContext有两个包装器作为其子容器。这些包装器也实现了Lifecycle接口。
    try {
